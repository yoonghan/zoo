...
describe("About Us", () => {
		...
		expectedHeaders.forEach((headers) => {
			expect(
				screen.getByRole("heading", {
					name: translations.aboutUs[headers].title,
				}),
			).toBeInTheDocument()
		})
	...
	it("should have a class 'anchor-link-header' for sticky header handling", async () => {
		const componentContainer = await act(async () => {
			const { container } = render(
				<About params={Promise.resolve({ lng: "en" })} />,
			)
			return container
		})
		...
		expectedHeaders.forEach((miniLink) => {
			expect(
				componentContainer.querySelector(`#${miniLink}`),
			).toHaveClass("anchor-link-header")
		})
	})
	...
	it("should generate site headers", async () => {
		...
		expect(metadata.description).toBe(
			translations.headers.aboutUs.description,
		)
	})
})