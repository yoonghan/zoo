...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
	...
	return generateSiteMeta(
		lng,
		t("headers.sitemap.title"),
		t("headers.sitemap.description"),
	)
}
...
const SiteMap = ({ t, lng }: Readonly<TranslatorProps>) => {
	function translatePath(path: string) {
		...
		if (givenPathName === "") return trimTitle(t(`headers.default`))

		const camelCasePath = givenPathName.replace(/-([a-z])/g, (_, char) =>
			char.toUpperCase(),
		)
				...
				{paths.map((path, index) => (
					<Fragment key={`${index}-${path}`}>
						<li className={`pb-2 ${calcMargin(path)}`}>
							<Link href={sanitizePath(`/${lng}${path}`)}>
								{translatePath(path)}
							</Link>
						</li>
					</Fragment>
