...
describe("Contact Us", () => {
	it("should contains important keys", async () => {
		...
		//h1
		expect(
			screen.getByRole("heading", { name: "Contact Zoo Negara" }),
		).toBeInTheDocument()

		//link to official site
		expect(screen.getByRole("link", { name: "1" })).toHaveAttribute(
			"href",
			"https://zoonegara.my/contact.html",
		)
	})
	...
	it("should contain phone numbers", async () => {
		...
		expect(
			screen.getByRole("link", { name: phoneNumber1 }),
		).toHaveAttribute("href", `tel:${phoneNumber1}`)

		expect(
			screen.getByRole("link", { name: phoneNumber2 }),
		).toHaveAttribute("href", `tel:${phoneNumber2}`)
	})
			...
			administrations.forEach((administration) => {
				expect(
					screen.getByRole("heading", {
						name: translations.contactUs.translate.Administration,
					}),
				).toBeInTheDocument()
				expect(
				...
				expect(
					screen.getByText(
						translations.contactUs.translate
	...
	it("should generate site headers", async () => {
		...
		expect(metadata.description).toBe(
			translations.headers.contactUs.description,
		)
	})
})