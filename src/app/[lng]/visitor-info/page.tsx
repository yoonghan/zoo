...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
	...
	return generateSiteMeta(
		lng,
		t("headers.visitorInfo.title"),
		t("headers.visitorInfo.description"),
	)
}
	...
	return (
		<>
			...
			<div className="italic text-sm mt-8">
				(*{" "}
				{t("footer.lastAdmission", {
					time: zooProfile.operatingTime.lastAdmissionTime,
				})}
				)
			</div>
		</>
...
function VisitorInfo({ t, lng }: Readonly<TranslatorProps>) {
	const rentals = t("visitorInfo.rental.facilities", {
		returnObjects: true,
	}) as {
		title: string
		description: string
	...
	const additionalInformationNotes = t(
		"visitorInfo.additionalInformationNotes",
		{ returnObjects: true },
	) as string[]
		...
		<>
			...
			<main>
				...
				>
					...
					<div className="mt-4">
						{t("visitorInfo.openingHours.description")}
					</div>
				...
				>
					...
					<div className="mt-8">
						<BuyTicketButton
							...
							alert={{
								...
								message: t(
									"translation.buyTicket.alert.message",
								),
								okBtnText: t(
									"translation.buyTicket.alert.confirm",
								),
							}}
							lng={lng}
						/>
					...
					<div className="mt-16 text-left">
						<strong>
							{t("visitorInfo.additionalInformationText")}:
						</strong>
						<ol className="list-decimal ml-6">
							<li>
								{t(
									"visitorInfo.admissionTicket.additionalInformation.important",
								)}
							</li>
							<li>
								<i className="text-yellow-700">
									{t(
										"visitorInfo.admissionTicket.additionalInformation.free",
									)}
								</i>{" "}
								-{" "}
								{t(
									"visitorInfo.admissionTicket.additionalInformation.freeOne",
								)}
							</li>
							<li>
								<i className="text-yellow-700">
								...
								-{" "}
								{t(
									"visitorInfo.admissionTicket.additionalInformation.freeTwo",
								)}
							</li>
						</ol>
					</div>
				...
				>
					...
					<div className="mt-4">
						<div>
							{t("visitorInfo.tramRide.priceTable.description")}
						</div>
						<table className="mt-2 w-full">
							<thead>
								<tr>
									<th>
										{t(
											"visitorInfo.tramRide.priceTable.h1",
										)}
									</th>
									<th>
										{t(
											"visitorInfo.tramRide.priceTable.h2",
										)}
									</th>
									<th>
										{t(
											"visitorInfo.tramRide.priceTable.h3",
										)}
									</th>
								</tr>
							</thead>
							<tbody>
								<tr>
									<td>
										{t(
											"visitorInfo.tramRide.priceTable.adult",
										)}
									</td>
									<td>MYR 10.90</td>
									<td>MYR 14.90</td>
								...
								<tr>
									<td>
										{t(
											"visitorInfo.tramRide.priceTable.children",
										)}
									</td>
									<td>MYR 6.90</td>
									<td>MYR 11.90</td>
						...
						<div className="mt-8">
							...
							<ol className="list-decimal ml-6">
								{additionalInformationNotes.map(
									(additionalInformation) => (
										<li key={additionalInformation}>
											{additionalInformation}
										</li>
									),
								)}
