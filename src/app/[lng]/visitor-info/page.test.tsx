...
describe("Visitor Info", () => {
	it("should contains important keys", async () => {
		...
		expect(
			screen.getByRole("heading", {
				name: translations.visitorInfo.title,
			}),
		).toBeInTheDocument()
		...
		expect(
			screen.getByRole("heading", {
				name: translations.visitorInfo.openingHours.title,
			}),
		).toBeInTheDocument()
		expect(
			screen.getByRole("heading", {
				name: translations.visitorInfo.admissionTicket.title,
			}),
		).toBeInTheDocument()
		expect(
			screen.getByRole("heading", {
				name: translations.visitorInfo.rental.title,
			}),
		).toBeInTheDocument()
		expect(
			screen.getByRole("heading", {
				name: translations.visitorInfo.tramRide.title,
			}),
		).toBeInTheDocument()
		...
		expect(
			screen.getByText(
				translations.visitorInfo.rental.facilities[0].title,
			),
		).toBeInTheDocument()
		expect(
			screen.getByText(
				translations.visitorInfo.additionalInformationNotes[0],
			),
		).toBeInTheDocument()
		...
		expect(
			screen.getByText(
				`(* ${mainTranslations.footer.lastAdmission})`.replace(
					"{{time}}",
	...
	it("should generate site headers", async () => {
		...
		expect(metadata.description).toBe(
			translations.headers.visitorInfo.description,
		)
	})
})