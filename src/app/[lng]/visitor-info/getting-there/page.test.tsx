...
describe("Getting There", () => {
	it("should contains important keys", async () => {
		...
		expect(
			screen.getByRole("heading", { name: "Train" }),
		).toBeInTheDocument()
		expect(
			screen.getByRole("heading", { name: "Public Bus" }),
		).toBeInTheDocument()

		expect(
			screen.getByRole("link", {
				name: translations.gettingThere.viewBtnText,
			}),
		).toBeInTheDocument()
	})
	...
	it("should generate site headers", async () => {
		...
		expect(metadata.description).toBe(
			translations.headers.gettingThere.description,
		)
	})
})