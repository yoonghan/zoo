import { act, render, screen } from "@testing-library/react"
import en from "@/i18n/locales/en/translation"
import { withComponentTranslator } from "./withComponentTranslator"
import { type TranslatorProps, withTranslator } from "./withTranslator"

describe("withComponentTranslator", () => {
	it("should return a component that renders with array", async () => {
		const keyword = "announcements"

		const SubTranslatedComponent = withComponentTranslator(
			({ t, lng }: Readonly<TranslatorProps>) => {
				const announcements: string[] = t(keyword, {
					returnObjects: true,
				}) as string[]
				return (
					<>
						<div>{lng}</div>
						<div>
							{announcements.map((a) => (
								<span key={a}>{a}</span>
							))}
						</div>
					</>
				)
			},
		)
		const TestComponent = ({ t, lng }: Readonly<TranslatorProps>) => (
			<SubTranslatedComponent t={t} lng={lng} />
		)

		const TranslatedComponent = withTranslator(TestComponent)
		await act(async () => {
			render(
				<TranslatedComponent params={Promise.resolve({ lng: "en" })} />,
			)
		})

		expect(await screen.findByText(en[keyword][0])).toBeInTheDocument()
		expect(await screen.getByText("en")).toBeInTheDocument()
	})
})