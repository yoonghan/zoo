...
describe("MiniMenu", () => {
	...
	it("should render model with hash", () => {
		...
		expect(
			screen.getByRole("link", { name: "Five Pillars" }),
		).toHaveAttribute("href", "#five-pillars")
	})

	it("should be able to fake ref", () => {
		render(
			<MiniMenu
				model={[{ hashId: "about-us", title: "About Us" }]}
				fakeNavBarRef={true}
			/>,
		)
		fireEvent.scroll(window, {})
	...
	describe("sticky menu", () => {
		...
		it("should add sticky class when scrolled over a distance", () => {
			...
			expect(
				screen.getByRole("navigation").classList.contains("sticky"),
			).toBeFalsy()
	...
	describe("link click", () => {
		it("should call scrollToView to horizontally scroll to itself and italize it", async () => {
			renderComponent()
			const fivePillarsElem = screen.getByRole("link", {
				name: "Five Pillars",
			})
