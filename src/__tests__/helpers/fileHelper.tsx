...
// biome-ignore lint/suspicious/noExportsInTest: Allowed for reuse
...
export const isNextJsImageAssetExist = (imgSrc: string) => {
	const removedLeadingNextFormat = imgSrc.replace(
...
describe("fileHelper", () => {
	describe("checkDownloadLinkHasHostAllLocalFiles", () => {
		const renderLinksWithDownload = () =>
			render(
				<div>
					...
					<a
						href="http://localhost/docs/notfound.pdf"
						download={true}
					>
						Not Found Doc
					</a>
		...
		it("should display 1 file missing and the other shown", () => {
			...
			const status = checkDownloadLinkHasHostAllLocalFiles(
				result.container,
			)
		...
		it("should be able to check _nextjs image", () => {
			expect(
				isNextJsImageAssetExist(
					"/_next/image?url=%2Fimages%2Fzoo-negara-map-1.webp&w=3840&q=75",
				),
			).toBeTruthy()
		})
	})
